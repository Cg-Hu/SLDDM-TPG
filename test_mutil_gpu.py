...
def load_model_from_config(config, ckpt, verbose=False):
    ...
    model = torch.nn.DataParallel(model, device_ids=[0, 1])  # 使用 DataParallel 在 2 号和 3 号 GPU 上并行
    model.cuda()
    model.eval()
    return model
...
def main():
    ...
    parser.add_argument(
        ...
        default="/nfs5/hcg/datasets/slddm-evaluation/train"
    )
    parser.add_argument(
        "--skip_grid",
        action='store_true',
    ...
    dataset = CTPDataset(opt.dataroot, opt.H, mode='train', unpaired=opt.unpaired)
    ...
    with torch.no_grad():
        with precision_scope("cuda"):
            with model.module.ema_scope():  # 使用 model.module 访问 DataParallel 包裹的模型
                for data in iterator:
                    ...
                    uc = None
                    if opt.scale != 1.0:
                        uc = model.module.learnable_vector
                        uc = uc.repeat(ref_tensor.size(0), 1, 1)
                    c = model.module.get_learned_conditioning(ref_tensor.to(torch.float16))
                    c = model.module.proj_out(c)

                    z_inpaint = model.module.encode_first_stage(test_model_kwargs['inpaint_image'])
                    z_inpaint = model.module.get_first_stage_encoding(z_inpaint).detach()
                    test_model_kwargs['inpaint_image'] = z_inpaint
                    test_model_kwargs['inpaint_mask'] = Resize([z_inpaint.shape[-2], z_inpaint.shape[-1]])(
                        test_model_kwargs['inpaint_mask'])

                    warp_feat = model.module.encode_first_stage(feat_tensor)
                    warp_feat = model.module.get_first_stage_encoding(warp_feat).detach()

                    ts = torch.full((1,), 999, device=device, dtype=torch.long)
                    start_code = model.module.q_sample(warp_feat, ts)
