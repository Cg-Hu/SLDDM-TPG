...
class CustomModelCheckpoint(ModelCheckpoint):
    def on_save_checkpoint(self, trainer, pl_module, checkpoint):
        # 筛选 state_dict，只保留包含 'ldn' 的层的权重
        filtered_state_dict = {k: v for k, v in checkpoint['state_dict'].items() if ('ldn' in k and 'model.diffusion_model' in k) or 'proj_out_ldn' in k or 'learnable_vector' in k}

        # 将筛选后的 state_dict 设置到 checkpoint 中
...
class SetupCallback(Callback):
    ...
    def on_pretrain_routine_start(self, trainer, pl_module):
        if trainer.global_rank == 0: # 有助于管理分布式训练中的进程，确保一些重要操作只在主进程中进行
...
class ImageLogger(Callback):
    def __init__(self, batch_frequency, max_images, clamp=True, increase_log_steps=True,
        ...
        self.log_steps = [2 ** n for n in range(int(np.log2(self.batch_freq)) + 1)] # 每次进行采样的时间步骤
...
if __name__ == "__main__":
    ...
    now = (time_delta + datetime.datetime.now()).strftime("%Y-%m-%dT%H:%M:%S")
    sys.path.append(os.getcwd())

    parser = get_parser()
    parser = Trainer.add_argparse_args(parser) # 将 Trainer 类中定义的参数配置选项添加到一个已有的 ArgumentParser 对象 parser 中，这样可以方便地在命令行中通过参数来配置模型训练过程中的一些设置
    ...
    if opt.resume: # 这是从检查点logdir恢复训练
        ...
        if os.path.isfile(opt.resume):
            ...
            # 根据所用的权重重新开一个
            print("重新开一个文件夹")
            path = paths[-1] # 代表使用哪个权重去resume
            nowname = now  + opt.postfix
            logdir = os.path.join(opt.logdir, f"{nowname}_{path}")
            print(logdir)
        else:
        ...
        if opt.name:
            name = "_" + opt.name
        elif opt.base: # viton512.yaml
            cfg_fname = os.path.split(opt.base[0])[-1]
            cfg_name = os.path.splitext(cfg_fname)[0]
            name = "_" + cfg_name + "occupy"
        else:
            name = ""
        ckpt_name = opt.pretrained_model.split('/')[-1]
        nowname = now + name + opt.postfix + ckpt_name
    ...
    # default to ddp
    trainer_config["accelerator"] = "ddp" # 分布式训练
    ...
    if not opt.resume:
        if opt.train_from_scratch:
            ...
            model.load_state_dict(ckpt_file, strict=True)
    ...
    if "logger" in lightning_config: # config中的trainer部分
        logger_cfg = lightning_config.logger
    else:
        logger_cfg = OmegaConf.create()
    logger_cfg = OmegaConf.merge(default_logger_cfg, logger_cfg)
    trainer_kwargs["logger"] = instantiate_from_config(logger_cfg) # pytorch_lightning.loggers.TestTubeLogger
    ...
    data.prepare_data()
    data.setup() # 这一步已经把所有的数据path和信息都注册好了
    print("#### Data #####")
