...
from sldm.modules.diffusionmodules.util import make_ddim_sampling_parameters, make_ddim_timesteps, noise_like, \
    extract_into_tensor
...
class DDIMSampler(object):
    ...
    def p_sample_ddim(self, x, c, t, index, repeat_noise=False, use_original_steps=False, quantize_denoised=False,
        ...
        cond_cpsd = kwargs['cond_cpsd'] # 是一个列表这个条件
        if 'test_model_kwargs' in kwargs:
            kwargs=kwargs['test_model_kwargs']
            x = torch.cat([x, kwargs['inpaint_image'], kwargs['inpaint_mask']],dim=1) # 在这里拼接了最终的输入
        elif 'rest' in kwargs:
        ...
        if unconditional_conditioning is None or unconditional_guidance_scale == 1.:
            e_t = self.model.apply_model(x, t, c, cond_cpsd=cond_cpsd)
        else:
            x_in = torch.cat([x] * 2)
            t_in = torch.cat([t] * 2)
