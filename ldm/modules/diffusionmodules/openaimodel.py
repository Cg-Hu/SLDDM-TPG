...
class TimestepEmbedSequential(nn.Sequential, TimestepBlock):
    ...
    def forward(self, x, emb, context=None, context_ldn=None):
        for layer in self:
            if isinstance(layer, TimestepBlock):
                x = layer(x, emb)
            elif isinstance(layer, SpatialTransformer):
                x = layer(x, context, context_ldn) # 这个先走attention.py的SpatialTransformer 261行
            else:
                x = layer(x)
        return x
...
class UNetModel(nn.Module):
    ...
    def forward(self, x, timesteps=None, context=None, context_ldn=None, x_diff=None, x_affine=None, y=None,**kwargs):
        """
        ...
        for module in self.input_blocks:
            h = module(h, emb, context, context_ldn=context_ldn)
            hs.append(h)
        h = self.middle_block(h, emb, context, context_ldn=context_ldn) # [bt, 1280, 8, 8]
